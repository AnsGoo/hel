import { getGlobalThis } from './globalRef';

export function getOB(): any {
  const gs = getGlobalThis();
  return gs.MutationObserver || gs.WebKitMutationObserver || gs.MozMutationObserver;
}

/**
 * 经测试 nodeList.find(node => node.tagName === 'BODY')
 * 可能在某些浏览器报错，故抽象为 for 循环语法方便多处复用
 */
export function findNode(nodeList: NodeList, tagName: string): Node | null {
  let targetNode: Node | null = null;
  const len = nodeList.length;
  for (let i = 0; i < len; i++) {
    const node = nodeList[i];
    if (node.tagName === tagName) {
      targetNode = node;
      break;
    }
  }
  return targetNode;
}

export function okeys<T extends object>(map: T): Array<keyof T> {
  return Object.keys(map) as Array<keyof T>;
}

function getSearch(): string {
  const getLocSearch = (location: Location | undefined): string => {
    const locVar = location || {};
    let search = locVar.search || '';
    if (!search) {
      const hash = locVar.hash || '';
      const hashSearch = hash.split('?')[1] || '';
      if (hashSearch) {
        search = `?${hashSearch}`;
      }
    }
    return search;
  };

  try {
    return getLocSearch(getGlobalThis()?.top?.location);
  } catch (err) {
    // 可能是非同域的 iframe 载入，访问 iframe 外部变量导致的报错
    return getLocSearch(getGlobalThis()?.location);
  }
}

export function getLsItem(key: string): string | null | undefined {
  const ls = getGlobalThis()?.localStorage;
  return ls?.getItem(key);
}

export function setLsItem(key: string, val: string): void {
  const ls = getGlobalThis()?.localStorage;
  ls?.setItem(key, val);
}

export function getSearchObj(): Record<string, string> {
  const search = getSearch();
  const map: Record<string, string> = {};
  if (search?.startsWith('?')) {
    const pureSearch = search.substring(1);
    const items = pureSearch.split('&');
    items.forEach((item) => {
      const [key, value] = item.split('=');
      map[key] = value;
    });
  }
  return map;
}

export function getJsRunLocation(): string {
  let loc = '';
  try {
    throw new Error('getJsRunLocation');
  } catch (err: any) {
    const stackArr: string[] = err.stack.split('\n');
    const len = stackArr.length;
    // firefox 下 len-1 取不到，需要取 len-2
    loc = stackArr[len - 1] || stackArr[len - 2] || '';
  }
  return loc;
}

export function setSubMapValue<T extends object>(rootObj: T, key: keyof T, subKey: string, subValue: any): void {
  const subMap = safeGetMap(rootObj, key);
  subMap[subKey] = subValue;
}

export function safeGetMap<T extends object, K extends keyof T>(rootObj: T, key: K, defaultMap: any = {}): any {
  let subMap = rootObj[key];
  if (!subMap) {
    subMap = defaultMap;
    rootObj[key] = subMap;
  }
  return subMap;
}

export function safeAssign<T extends object>(assignTo: T, assignFrom: Partial<T>): void {
  Object.keys(assignFrom).forEach((key) => {
    const val = assignFrom[key as keyof T];
    if (![null, undefined, ''].includes(val)) {
      assignTo[key as keyof T] = val;
    }
  });
}

export function noDupPush<T>(oriList: T[], toPush: T): void {
  if (!oriList.includes(toPush)) oriList.push(toPush);
}

interface IsNullOptions {
  nullValues?: Array<any>;
  emptyObjIsNull?: boolean;
  emptyArrIsNull?: boolean;
}

export function isNull(value: any, nullDef: IsNullOptions = {}): boolean {
  const { nullValues = [null, undefined, ''], emptyObjIsNull = true, emptyArrIsNull = true } = nullDef;

  const inNullValues = nullValues.includes(value);
  if (inNullValues) {
    return true;
  }

  if (Array.isArray(value)) {
    if (emptyArrIsNull) return value.length === 0;
    return false;
  }

  if (typeof value === 'object' && value !== null) {
    const keys = okeys(value);
    const keyLen = keys.length;
    if (emptyObjIsNull) return keyLen === 0;
    return false;
  }

  return false;
}
