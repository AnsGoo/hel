...
function doAppend(nativeAppend: Function, el: HTMLLinkElement | HTMLScriptElement): any {
  if (!el || !['LINK', 'SCRIPT'].includes(el.tagName)) {
    return nativeAppend(el);
  }
  const { href, tagName, src } = el;
  const url: string = href || src || '';
  ...
  if (beforeAppend) {
    ...
    const setAssetUrl = (newUrl: string) => el.setAttribute(urlKey, newUrl);
...
export function patchAppendChild(): void {
  ...
  if (body) {
    ...
    const observer = new MutationObserver(function (mutations: MutationRecord[]) {
      ...
      if (body) {
        nativeBodyAppend = (body as HTMLBodyElement).appendChild.bind(body);
        helMicroShared.nativeBodyAppend = nativeBodyAppend;
        (body as HTMLBodyElement).appendChild = (el: any) => doAppend(getAppend(nativeBodyAppend, body, 'body'), el);
        observer.disconnect();
      }
    });
  ...
  const getAppend = function getAppend(nativeAppend: Function, bindTarget: any, loc: string): Function {
  ...
  // replace appendChild
  head.appendChild = (el: any) => doAppend(getAppend(nativeHeadAppend, head, 'head'), el);
  if (body) {
    body.appendChild = (el: any) => doAppend(getAppend(nativeBodyAppend, body, 'body'), el);
  }
}
