...
function matchIgnoreCssPrefix(node: HTMLElement, url: string): string {
...
function resetStyleText(groupName: string): void {
...
function handleStyleAdded(node: HTMLElement, ignoreStyleTagMap: Record<string, number>): void {
...
function handleHotUpdate(node: HTMLElement, ignoreStyleTagMap: Record<string, number>): boolean {
...
export function obStyleTagInsert(): void {
  ...
  const observer = new MutationObserver(function (mutations: MutationRecord[]) {
    mutations.forEach(function (mutation) {
      ...
      for (let i = 0; i < len; i++) {
        handleStyleAdded(addedNodes[i] as HTMLElement, ignoreStyleTagMap);
      }

      const remLen = removedNodes.length;
      for (let i = 0; i < remLen; i++) {
        if (handleHotUpdate(removedNodes[i] as HTMLElement, ignoreStyleTagMap)) {
          break;
        }
      }
    });
