...
/** @typedef {import('../base/microShared').SharedCache} SharedCache */
 ...
 */
export function getPureConfig(mayCache: SharedCache, needOrigin: boolean = false): any {
  const {
    apiMode,
    apiPrefix,
    apiSuffix,
...
export function getPlatformConfig(iPlatform: string): any {
  const cache = getSharedCache(iPlatform);
  return getPureConfig(cache, true);
}

export function initGlobalConfig(config: any): void {
  ...
  const errTip = (label: string) => `pass same ${label} handle, hel will ignore it for avlid 'Maximum call stack size exceeded'`;
 ...
 * @returns
 */
export function initPlatformConfig(config: any, iPlatform: string): void {
  const cache = getSharedCache(iPlatform);
...
export function originInit(platform: string, options: any): void {
 ...
 * @returns
 */
export function getAppPlatform(appGroupName: string): string {
 ...
 */
export function setAppPlatform(appGroupName: string, platform: string, overwrite: boolean): string {
...
export function getAppPlatformByAppName(appName: string): string {
 ...
 */
export function setAppPlatformByAppName(appName: string, platform: string, overwrite: boolean): string {
