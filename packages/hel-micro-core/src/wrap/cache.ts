import { getHelMicroShared, makeCacheNode, SharedCache } from '../base/microShared';
 ...
 */
export function getSharedCache(platform?: string): SharedCache {
...
export function getCacheRoot(): any {
  const shared = getHelMicroShared();
  if (!shared) {
    throw new Error('hel micro shared not initialized');
  }
  return shared.cacheRoot;
}
